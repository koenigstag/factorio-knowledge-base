...
🚧 Early stage. Currently populated: sourced game facts (`datapacks/`, `mechanics/`), derived relations (`relations/`), terminology (`glossary/`), base-layout patterns (`layouts/`), and curated importable blueprints (`blueprints/curated/`, parsed via `blueprints/codec.py`). Automated blueprint generation and `patterns/` are not built yet.
